Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueRelationMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueRelationMainController',
     ...
     init: function() {
          ...
          this.issueIdLoad();
          this.issueRelationIdLoad();
          this.relationCodeLoad();
     },
     issueIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/IssueWorkflow/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#issueId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'issueId', response.responseText);
     ...
     issueRelationIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#issueRelationId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'issueRelationId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#issueRelationId');
     ...
     relationCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/IssueRelationType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#relationCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'relationCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#relationCode');
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueRelationTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueRelationForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueRelationGrid');
               var tree = this.view.down('#IssueRelationTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueRelationForm');
          var grid = this.view.down('#IssueRelationGrid');
          var tree = this.view.down('#IssueRelationTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueRelationGrid');
               ...
               var formPanel = this.getView().up().down('#IssueRelationForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueRelationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueRelationForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueRelationGrid').store;
          gridStore.removeAll();
