Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueActivityMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueActivityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueActivityTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueActivityForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueActivityGrid');
               var tree = this.view.down('#IssueActivityTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueActivityForm');
          var grid = this.view.down('#IssueActivityGrid');
          var tree = this.view.down('#IssueActivityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueActivityGrid');
               ...
               var formPanel = this.getView().up().down('#IssueActivityForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueActivityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueActivityForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueActivityGrid').store;
          gridStore.removeAll();
