Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueStageMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueStageMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueStageTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueStageForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueStageGrid');
               var tree = this.view.down('#IssueStageTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueStageForm');
          var grid = this.view.down('#IssueStageGrid');
          var tree = this.view.down('#IssueStageTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueStageGrid');
               ...
               var formPanel = this.getView().up().down('#IssueStageForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueStageTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueStageForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueStageGrid').store;
          gridStore.removeAll();
