Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.IssueVisibilityMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueVisibilityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueVisibilityTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueVisibilityForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueVisibilityGrid');
               var tree = this.view.down('#IssueVisibilityTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueVisibilityForm');
          var grid = this.view.down('#IssueVisibilityGrid');
          var tree = this.view.down('#IssueVisibilityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueVisibilityGrid');
               ...
               var formPanel = this.getView().up().down('#IssueVisibilityForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueVisibilityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueVisibilityForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueVisibilityGrid').store;
          gridStore.removeAll();
