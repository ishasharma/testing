Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.FeatureCategoryMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.FeatureCategoryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#FeatureCategoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#FeatureCategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#FeatureCategoryGrid');
               var tree = this.view.down('#FeatureCategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#FeatureCategoryForm');
          var grid = this.view.down('#FeatureCategoryGrid');
          var tree = this.view.down('#FeatureCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#FeatureCategoryGrid');
               ...
               var formPanel = this.getView().up().down('#FeatureCategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#FeatureCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#FeatureCategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#FeatureCategoryGrid').store;
          gridStore.removeAll();
