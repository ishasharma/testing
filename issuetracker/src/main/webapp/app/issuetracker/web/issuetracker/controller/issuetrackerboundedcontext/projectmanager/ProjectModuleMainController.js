Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.ProjectModuleMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.ProjectModuleMainController',
     init: function() {
          var form = this.view.down('#ProjectModuleForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#ProjectModuleTree');
               ...
               var compRef = this.view.down('#ProjectModuleTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#ProjectModuleGrid');
          var tree = this.view.down('#ProjectModuleTree');
          tree.setSelection();
          grid.setSelection();
          var gridModuleUserMapping = but.up('form').down('#ModuleUserMappingGrid');
          gridModuleUserMapping.getStore().removeAll();
          gridModuleUserMapping.reconfigure();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#ProjectModule');
          var grid = this.view.down('#ProjectModuleGrid');
          var tree = this.view.down('#ProjectModuleTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ProjectModuleForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#ProjectModuleTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var ProjectModule = this.view.down('#ProjectModuleForm');
               ProjectModule.reset();
               var gridPanel = this.view.down('#ProjectModuleGrid');
          ...
          var formPanel = this.getView().up().down('#ProjectModuleForm');
          ...
          var ModuleUserMappingGrid = formPanel.down('#ModuleUserMappingGrid');
          ModuleUserMappingGrid.store.loadData(data.moduleUserMapping);
          ModuleUserMappingGrid.setSelection(0);
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ProjectModuleTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ProjectModule').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ProjectModuleGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     addModuleUserMapping: function(but) {
          var formModuleUserMapping = but.up().down('form');
          var gridModuleUserMapping = but.up().up().down('#ModuleUserMappingGrid');
          if (formModuleUserMapping.isValid()) {
               var values = this.getData(formModuleUserMapping);
               values = values[gridModuleUserMapping.bindable];
               if (gridModuleUserMapping.selection) {
                    gridModuleUserMapping.selection.data = values;
               } else {
                    gridModuleUserMapping.getStore().add(values);
               }
               gridModuleUserMapping.reconfigure();
               formModuleUserMapping.reset();
               gridModuleUserMapping.setSelection();
          }
     },
     onModuleUserMappingGridItemClick: function(cellModel, record, rowIndex, columnIndex, eOpts, comp) {
          var form;
          if (comp != null) {
               form = comp.up();
          } else {
               form = this.view.down('#ModuleUserMappingForm');
          }
     ...
     saveForm: function(but, evt) {
          ...
          var ModuleUserMappingForm = form.down('#ModuleUserMappingForm');
          var ModuleUserMapping = form.down('#ModuleUserMappingGrid');
          if (!(ModuleUserMappingForm.isValid() || ModuleUserMapping.store.getCount() != 0)) {
               this.showCard(form, ModuleUserMappingForm);
               return;
          }
          this.addModuleUserMapping(ModuleUserMappingForm.down('button'));
