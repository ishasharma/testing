Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssuePriorityMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssuePriorityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssuePriorityTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssuePriorityForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssuePriorityGrid');
               var tree = this.view.down('#IssuePriorityTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssuePriorityForm');
          var grid = this.view.down('#IssuePriorityGrid');
          var tree = this.view.down('#IssuePriorityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssuePriorityGrid');
               ...
               var formPanel = this.getView().up().down('#IssuePriorityForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssuePriorityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssuePriorityForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssuePriorityGrid').store;
          gridStore.removeAll();
