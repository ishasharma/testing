Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.WorkflowUserRoleMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.WorkflowUserRoleMainController',
     ...
     workflowIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/Workflow/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#workflowId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'workflowId', response.responseText);
     ...
     prjRoleIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/ProjectRoles/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#prjRoleId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'prjRoleId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#prjRoleId');
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#WorkflowUserRoleTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#WorkflowUserRoleForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#WorkflowUserRoleGrid');
               var tree = this.view.down('#WorkflowUserRoleTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#WorkflowUserRoleForm');
          var grid = this.view.down('#WorkflowUserRoleGrid');
          var tree = this.view.down('#WorkflowUserRoleTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#WorkflowUserRoleGrid');
               ...
               var formPanel = this.getView().up().down('#WorkflowUserRoleForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#WorkflowUserRoleTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#WorkflowUserRoleForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#WorkflowUserRoleGrid').store;
          gridStore.removeAll();
