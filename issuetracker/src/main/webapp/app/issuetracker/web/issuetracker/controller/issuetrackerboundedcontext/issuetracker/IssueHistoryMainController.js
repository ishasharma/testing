Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueHistoryMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueHistoryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueHistoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueHistoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueHistoryGrid');
               var tree = this.view.down('#IssueHistoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueHistoryForm');
          var grid = this.view.down('#IssueHistoryGrid');
          var tree = this.view.down('#IssueHistoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueHistoryGrid');
               ...
               var formPanel = this.getView().up().down('#IssueHistoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueHistoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueHistoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueHistoryGrid').store;
          gridStore.removeAll();
