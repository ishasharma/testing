Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueRelationTypeMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueRelationTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueRelationTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueRelationTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueRelationTypeGrid');
               var tree = this.view.down('#IssueRelationTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueRelationTypeForm');
          var grid = this.view.down('#IssueRelationTypeGrid');
          var tree = this.view.down('#IssueRelationTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueRelationTypeGrid');
               ...
               var formPanel = this.getView().up().down('#IssueRelationTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueRelationTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueRelationTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueRelationTypeGrid').store;
          gridStore.removeAll();
