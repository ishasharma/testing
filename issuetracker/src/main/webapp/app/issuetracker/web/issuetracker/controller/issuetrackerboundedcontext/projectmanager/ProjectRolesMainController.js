Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.ProjectRolesMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ProjectRolesMainController',
     ...
     init: function() {
          ...
          this.issueVisibilityCodeLoad();
     },
     issueVisibilityCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/IssueVisibility/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#issueVisibilityCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'issueVisibilityCode', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ProjectRolesTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ProjectRolesForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ProjectRolesGrid');
               var tree = this.view.down('#ProjectRolesTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ProjectRolesForm');
          var grid = this.view.down('#ProjectRolesGrid');
          var tree = this.view.down('#ProjectRolesTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ProjectRolesGrid');
               ...
               var formPanel = this.getView().up().down('#ProjectRolesForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ProjectRolesTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ProjectRolesForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ProjectRolesGrid').store;
          gridStore.removeAll();
