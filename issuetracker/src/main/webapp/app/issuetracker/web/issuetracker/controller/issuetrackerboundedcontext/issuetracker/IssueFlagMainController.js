Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueFlagMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueFlagMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueFlagTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueFlagForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueFlagGrid');
               var tree = this.view.down('#IssueFlagTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueFlagForm');
          var grid = this.view.down('#IssueFlagGrid');
          var tree = this.view.down('#IssueFlagTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueFlagGrid');
               ...
               var formPanel = this.getView().up().down('#IssueFlagForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueFlagTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueFlagForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueFlagGrid').store;
          gridStore.removeAll();
