Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueStatusMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueStatusMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueStatusTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueStatusForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueStatusGrid');
               var tree = this.view.down('#IssueStatusTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueStatusForm');
          var grid = this.view.down('#IssueStatusGrid');
          var tree = this.view.down('#IssueStatusTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueStatusGrid');
               ...
               var formPanel = this.getView().up().down('#IssueStatusForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueStatusTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueStatusForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueStatusGrid').store;
          gridStore.removeAll();
