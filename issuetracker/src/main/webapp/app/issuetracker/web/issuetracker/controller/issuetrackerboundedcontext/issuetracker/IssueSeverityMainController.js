Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.issuetracker.IssueSeverityMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IssueSeverityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IssueSeverityTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IssueSeverityForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IssueSeverityGrid');
               var tree = this.view.down('#IssueSeverityTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IssueSeverityForm');
          var grid = this.view.down('#IssueSeverityGrid');
          var tree = this.view.down('#IssueSeverityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IssueSeverityGrid');
               ...
               var formPanel = this.getView().up().down('#IssueSeverityForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#IssueSeverityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IssueSeverityForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IssueSeverityGrid').store;
          gridStore.removeAll();
