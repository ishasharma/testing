Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.ProjectAccessRightsMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ProjectAccessRightsMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ProjectAccessRightsTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ProjectAccessRightsForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ProjectAccessRightsGrid');
               var tree = this.view.down('#ProjectAccessRightsTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ProjectAccessRightsForm');
          var grid = this.view.down('#ProjectAccessRightsGrid');
          var tree = this.view.down('#ProjectAccessRightsTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ProjectAccessRightsGrid');
               ...
               var formPanel = this.getView().up().down('#ProjectAccessRightsForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ProjectAccessRightsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ProjectAccessRightsForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ProjectAccessRightsGrid').store;
          gridStore.removeAll();
