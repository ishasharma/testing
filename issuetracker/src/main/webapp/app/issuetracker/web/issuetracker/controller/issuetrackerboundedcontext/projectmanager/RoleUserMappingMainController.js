Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.RoleUserMappingMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.RoleUserMappingMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#RoleUserMappingTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#RoleUserMappingForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#RoleUserMappingGrid');
               var tree = this.view.down('#RoleUserMappingTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#RoleUserMappingForm');
          var grid = this.view.down('#RoleUserMappingGrid');
          var tree = this.view.down('#RoleUserMappingTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#RoleUserMappingGrid');
               ...
               var formPanel = this.getView().up().down('#RoleUserMappingForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#RoleUserMappingTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#RoleUserMappingForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#RoleUserMappingGrid').store;
          gridStore.removeAll();
