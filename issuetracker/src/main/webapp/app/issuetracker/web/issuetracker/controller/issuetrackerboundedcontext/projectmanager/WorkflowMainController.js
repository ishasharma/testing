Ext.define('Issuetracker.issuetracker.web.issuetracker.controller.issuetrackerboundedcontext.projectmanager.WorkflowMainController', {
     extend: 'Issuetracker.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.WorkflowMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#WorkflowTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#WorkflowForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#WorkflowGrid');
               var tree = this.view.down('#WorkflowTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#WorkflowForm');
          var grid = this.view.down('#WorkflowGrid');
          var tree = this.view.down('#WorkflowTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#WorkflowGrid');
               ...
               var formPanel = this.getView().up().down('#WorkflowForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#WorkflowTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#WorkflowForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#WorkflowGrid').store;
          gridStore.removeAll();
